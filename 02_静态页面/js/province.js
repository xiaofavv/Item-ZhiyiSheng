...
cityArray[34] = new Array("其它","北美洲|南美洲|亚洲|非洲|欧洲|大洋洲"); 
       ...
       function getCity2(currProvince) 
              { 
                  //当前 所选择 的 省 
                  var currProvincecurrProvince = currProvince; 
                  var i,j,k; 
                  //清空 城市 下拉选单 
                  document.all.selCity2.length = 0 ;  
                  for (i = 0 ;i <cityArray.length;i++) 
                    {    
                        //得到 当前省 在 城市数组中的位置 
                        if(cityArray[i][0]==currProvince) 
                          {    
                              ...
                              tmpcityArray = cityArray[i][1].split("|") 
                                ...
                                { 
                                  //填充 城市 下拉选单 
                                  document.all.selCity2.options[document.all.selCity2.length] = new Option(tmpcityArray[j],tmpcityArray[j]);  
                                } 
                          }  
                    }  
              } 
